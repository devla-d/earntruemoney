...
from accounts.models import Account,LoginHistory,Referral
from .models import *
    ...
    if request.POST:
        ...
        if user.balance > amount:
            bank = Bank.objects.create(
                acc_name=accountname,acc_num=accountnum,
                ty_pe=bank
            )
            transaction =  Transactions.objects.create(
                user=user,amount=amount,trans_type=utils.W,unique_u=utils.trans_code()
            )
    ...
    if request.POST:
        ...
        if user.deposit_balance >=  amount:
            if amount not in range(package.min_amount ,package.max_amount):
                messages.warning(request, (f'Input Amount Between Your Selected Plans Price'))
